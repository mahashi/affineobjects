\begin{definition}[Restrictive functor]\label[definition]{f13}
...
\begin{definition}[Restrictive morphism]\label[definition]{f1c}
...
\begin{example}\label[example]{f21}
In $\schemes$, the morphism $\Spec{A_f} \rightarrow \Spec{A}$ is restrictive.
% functor that is restrictive. Yoneda embedding? AffSch in Sch?
\end{example}

\begin{example}\label[example]{f26}
...
%  right it out completely
\end{example}

\begin{lemma}[Restrictive to affines]\label[lemma]{f2c}
If $f:X\rightarrow \Spec{R}$ is a restrictive open immersion. 
then $X$ is affine.
% What is an open immersion categorically?
\end{lemma}
\begin{proof}
...
\begin{example}[Affine non-restrictive map]\label[example]{f39}
