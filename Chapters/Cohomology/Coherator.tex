...
\begin{definition}\label[definition]{5e4}
...
\begin{definition}[Grothendieck Category]\label[definition]{5ea}
...
\begin{corollary}[\url{http://stacks.math.columbia.edu/tag/077K}]\label[corollary]{5f9}

Let $X$ be 'affine'. Then $\Qcoh{X}$ is a Grothendieck abelian category.
\end{corollary}

\begin{remark}[\url{http://stacks.math.columbia.edu/tag/077K}]\label[remark]{5fe}

Due to a result of Gabber, $\Qcoh{X}$ is a Grothendieck category for any scheme $X$.
\end{remark}

\begin{remark}\label[remark]{603}
...
\begin{definition}[Right Adjoint/coherator]\label[definition]{623}
...
\begin{lemma}\label[lemma]{628}
The functor $Q$ is right adjoint to the inclusion functor $i:\Qcoh{X}\rightarrow \cat{Mod(X)}$.
\end{lemma}

\begin{lemma}[\url{http://stacks.math.columbia.edu/tag/015Z}]\label[lemma]{62c}
