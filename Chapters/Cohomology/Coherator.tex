...
\begin{definition}\label[definition]{49d}
...
\begin{definition}[Grothendieck Category]\label[definition]{Grothendieck Category}
...
\begin{corollary}[\url{http://stacks.math.columbia.edu/tag/077K}]\label[corollary]{4b2}

Let $X$ be 'affine'. Then $\Qcoh{X}$ is a Grothendieck abelian category.
\end{corollary}

\begin{remark}[\url{http://stacks.math.columbia.edu/tag/077K}]\label[remark]{4b7}

Due to a result of Gabber, $\Qcoh{X}$ is a Grothendieck category for any scheme $X$.
\end{remark}

\begin{remark}\label[remark]{4bc}
...
\begin{definition}[Right Adjoint/coherator]\label[definition]{Right Adjoint/coherator}
...
\begin{lemma}\label[lemma]{4e1}
The functor $Q$ is right adjoint to the inclusion functor $i:\Qcoh{X}\rightarrow \cat{Mod(X)}$.
\end{lemma}

\begin{lemma}[\url{http://stacks.math.columbia.edu/tag/015Z}]\label[lemma]{4e5}
