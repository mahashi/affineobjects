...
\begin{definition}\label[definition]{45c}
...
\begin{definition}[Grothendieck Category]\label[definition]{462}
...
\begin{corollary}[\url{http://stacks.math.columbia.edu/tag/077K}]\label[corollary]{471}

Let $X$ be 'affine'. Then $\Qcoh{X}$ is a Grothendieck abelian category.
\end{corollary}

\begin{remark}[\url{http://stacks.math.columbia.edu/tag/077K}]\label[remark]{476}

Due to a result of Gabber, $\Qcoh{X}$ is a Grothendieck category for any scheme $X$.
\end{remark}

\begin{remark}\label[remark]{47b}
...
\begin{definition}[Right Adjoint/coherator]\label[definition]{49b}
...
\begin{lemma}\label[lemma]{4a0}
The functor $Q$ is right adjoint to the inclusion functor $i:\Qcoh{X}\rightarrow \cat{Mod(X)}$.
\end{lemma}

\begin{lemma}[\url{http://stacks.math.columbia.edu/tag/015Z}]\label[lemma]{4a4}
