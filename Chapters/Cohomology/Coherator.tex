...
\begin{definition}\label[definition]{3ba}
...
\begin{definition}[Grothendieck Category]\label[definition]{3c0}
...
\begin{corollary}[\url{http://stacks.math.columbia.edu/tag/077K}]\label[corollary]{3cf}

Let $X$ be 'affine'. Then $\Qcoh{X}$ is a Grothendieck abelian category.
\end{corollary}

\begin{remark}[\url{http://stacks.math.columbia.edu/tag/077K}]\label[remark]{3d4}

Due to a result of Gabber, $\Qcoh{X}$ is a Grothendieck category for any scheme $X$.
\end{remark}

\begin{remark}\label[remark]{3d9}
...
\begin{definition}[Right Adjoint/coherator]\label[definition]{3f9}
...
\begin{lemma}\label[lemma]{3fe}
The functor $Q$ is right adjoint to the inclusion functor $i:\Qcoh{X}\rightarrow \cat{Mod(X)}$.
\end{lemma}

\begin{lemma}[\url{http://stacks.math.columbia.edu/tag/015Z}]\label[lemma]{402}
