...
% Define $s_c(x) = (x_{i,j})_{i \in (S^j)_f, j \in }$
% where $S^j, T$ are considered as covering sieves on $fg:c\rightarrow a$ now.

% Fix $j$. Let $k: z \rightarrow \dom{j}$.
% We need $(x_{l,jk}) = (x_{p,j})$ where $l\in S_{jk}$ and $p\in k^*S^j$ are both covering sieves.
% These matching families agree on $S_{jk} \cap k^*S^j$, so they are the same because of the equivalence relation in the construction of the sheafification.