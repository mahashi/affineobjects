...
\begin{definition}[Presheaves]\label[definition]{7cf}
...
%%%%%%%%%%%%%%%%%  Section functors
\begin{definition}[Sections functor]\label[definition]{7d9}
...
\begin{example}\label[example]{7e8}
...
\begin{lemma}[Complete]\label[lemma]{7ee}
...
\begin{lemma}[Abelian/Grothendieck]\label[lemma]{7f6}
...
\begin{remark}\label[remark]{80c}
So $\presheaves{C}$ is a Grothendieck category.
\end{remark}

%%%%%%%%%%%%%%%%% Direct image/ Inverse image
\begin{definition}[direct image]\label[definition]{811}
...
\begin{lemma}[direct image commutes with limits]\label[lemma]{817}
...
\begin{definition}[Inverse image, direct image \& push-forward]\label[definition]{821}
...
See Stacks \href{http://stacks.math.columbia.edu/tag/09YX}{Tag 09YX} for a different existence lemma for the push-forward.
\end{definition}

%%%%%%%%%%%%%%%% Restricting and extending
Assume $\cat{C}$ has binary products.
\begin{definition}\label[definition]{830}
...
\begin{definition}[Internal Hom]\label[definition]{844}
...
\begin{definition}[Monoidal structure]\label[definition]{853}
...
\begin{lemma}[Adjunction/monoidal closed structure]\label[lemma]{858}
